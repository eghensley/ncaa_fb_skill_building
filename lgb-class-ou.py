...
N_FEATURES_OPTIONS = range(100, 1050, 50) #150
...
for k in N_FEATURES_OPTIONS:
    ...
    for i in range(1,40):
        ...
        for every in range(0, len(np.array(testj))):
            ...
            y_juice.append(ju)
        model = lgb.LGBMClassifier(min_child_samples = 5, random_state = 86, objective = 'binary_logloss', n_estimators = k, verbosity = 2, learning_rate = .021544346900318846/5)
