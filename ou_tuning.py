...
pipe = Pipeline([
    ...
    ('c_classify', SVC(kernel = 'rbf', random_state = 1108, probability = True, C = 1))
])
...
max_score = max(meanscores)
meanscores = meanscores.reshape(-1, len(param_b))
bar_offsets = (np.arange(len(param_b)) *
               (len(param_a) + 1) + .5)
for i, (label, reducer_scores) in enumerate(zip(param_a_names, meanscores)):
...
ax2.set_ylim((min(logloss_raw)*.9, max(logloss_raw)*1.05))
fig.tight_layout()
plt.show()
