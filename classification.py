...
pipe = Pipeline([
    ...
    ('reduce_dim', PCA(random_state = 1108, n_components=13,whiten=True,svd_solver='full')),
    ('classify', SVC(kernel = 'linear', random_state = 46, probability = True))
])
N_FEATURES_OPTIONS = np.logspace(-3, 2, 20)
#N_FEATURES_OPTIONS = range(2, 41)
...
grid = GridSearchCV(pipe, cv=StratifiedKFold(n_splits = 10, shuffle = True, random_state = 1108), n_jobs=-1, param_grid=param_grid, refit = False, scoring = scores, verbose = 3)
