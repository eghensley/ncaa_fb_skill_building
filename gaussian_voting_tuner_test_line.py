...
bounds = np.array([[0, 10], [0, 10], [0, 10], [0, 10],[0, 10], [0, 10]])
start  = [[1,1,1,1,1,1]]
...
results = bayesian_optimisation(n_iters=100, 
                      ...
                      x0 = start)
scores = results[1]
max(scores) = 0.60504201680672265
index = list(scores).index(max(scores))
results[0][index] = [ 8.23412644,  3.06938025,  7.83767065,  1.49649863,  2.07433432, 8.91300168]
